---
title: "BOSS DS-1 レビュー｜定番ディストーションの実力を検証"
description: "世界中のギタリストに愛される定番ディストーション、BOSS DS-1を徹底レビュー。初心者からプロまで使える理由に迫ります。"
category: effector
publishedAt: "2026-05-10"
rating: 4
thumbnail: "https://m.media-amazon.com/images/I/71WAFbpOI-L._AC_SL1500_.jpg"
products:
  - name: "BOSS DS-1 ディストーション"
    price: "¥7,700"
    amazonUrl: "https://www.amazon.co.jp/s?k=BOSS+DS-1&tag=gearnoteguitar-22"
    soundhouseUrl: "https://www.soundhouse.co.jp/"
    image: "https://m.media-amazon.com/images/I/71WAFbpOI-L._AC_SL1500_.jpg"
    specs:
      タイプ: "ディストーション"
      電源: "DC9V（乾電池006P/ACアダプター）"
      電流消費: "4mA"
      サイズ: "73×129×59mm"
      重量: "390g"
      入力インピーダンス: "1MΩ"
      出力インピーダンス: "1kΩ"
---

## 概要

1978年発売以来、世界中のステージやスタジオで愛用されてきたBOSS DS-1。コート・コバーンやスティーヴ・ヴァイなど名だたるギタリストも使用してきた本機を改めてレビューします。

## サウンドキャラクター

DS-1の特徴は、独特の「ザリザリ感」のある歪みです。単なるオーバードライブとは違う、マシンなディストーションサウンドが特徴。

## コントロール

シンプルな3ノブ設計：
- **LEVEL**：出力音量
- **TONE**：音のキャラクター（暗い〜明るい）
- **DIST**：歪み量

## 使い勝手

アナログ回路、単9V電源、堅牢な筐体。長年にわたって愛される理由がよく分かります。ライブでの信頼性は抜群です。

## まとめ

コスパ最強の定番エフェクター。迷ったらまずDS-1から始めてみましょう。
