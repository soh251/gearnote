---
title: "ディストーションとは？初心者に定番のBOSS DS-1など3選を解説"
description: "ディストーションのおすすめ3選を初心者向けに徹底解説。BOSS DS-1・ProCo RAT2・MXR M75の特徴を比較。オーバードライブとの違いも分かりやすく説明します。"
category: effector
publishedAt: "2026-06-04"
rating: 4
featured: false
thumbnail: "https://m.media-amazon.com/images/I/71WAFbpOI-L._AC_SL1500_.jpg"
products:
  - name: "BOSS DS-1 ディストーション"
    price: "¥9,350〜"
    amazonUrl: "https://www.amazon.co.jp/s?k=BOSS+DS-1&tag=gearnoteguitar-22"
    soundhouseUrl: "https://www.soundhouse.co.jp/search/index?search_all=BOSS+DS-1&i_type=a"
    ikebeUrl: "https://px.a8.net/svt/ejp?a8mat=4B5Q82+7EQ8KA+5KFA+BW0YB&a8ejpredirect=https%3A%2F%2Fwww.ikebe-gakki.com%2FForm%2FProduct%2FProductList.aspx%3Fshop%3D0%26cat%3D%26bid%3Dec%26dpcnt%3D20%26img%3D1%26sort%3D07%26udns%3D1%26fpfl%3D0%26sfl%3D0%26pno%3D1%26swrd%3DDS-1%2BBOSS"
    ishibashiUrl: "https://px.a8.net/svt/ejp?a8mat=4B5Q82+7FBO62+F14+BW0YB&a8ejpredirect=https%3A%2F%2Fstore.ishibashi.co.jp%2Fsearch%3Fsort%3DNumber1%26q%3Dboss%2Bds-1"
    image: "https://m.media-amazon.com/images/I/71WAFbpOI-L._AC_SL1500_.jpg"
    specs:
      タイプ: "ディストーション"
      電源: "DC9V / 乾電池006P"
      電流消費: "4mA"
      サイズ: "73×129×59mm"
      重量: "390g"
      コントロール: "LEVEL、TONE、DIST"
  - name: "ProCo RAT2"
    price: "¥14,900〜"
    amazonUrl: "https://www.amazon.co.jp/s?k=ProCo+RAT2&tag=gearnoteguitar-22"
    soundhouseUrl: "https://www.soundhouse.co.jp/search/index?search_all=ProCo+RAT2&i_type=a"
    ikebeUrl: "https://px.a8.net/svt/ejp?a8mat=4B5Q82+7EQ8KA+5KFA+BW0YB&a8ejpredirect=https%3A%2F%2Fwww.ikebe-gakki.com%2FForm%2FProduct%2FProductList.aspx%3Fshop%3D0%26cat%3D%26bid%3Dec%26dpcnt%3D20%26img%3D1%26sort%3D07%26udns%3D1%26fpfl%3D0%26sfl%3D0%26pno%3D1%26swrd%3DRAT2%2BPROCO"
    ishibashiUrl: "https://px.a8.net/svt/ejp?a8mat=4B5Q82+7FBO62+F14+BW0YB&a8ejpredirect=https%3A%2F%2Fstore.ishibashi.co.jp%2Fsearch%3Fsort%3DNumber1%26q%3Dproco%2Brat2"
    image: "https://m.media-amazon.com/images/I/81cBn3xtcDL._AC_SL1500_.jpg"
    specs:
      タイプ: "ディストーション"
      電源: "DC9V / 乾電池006P"
      電流消費: "5mA"
      コントロール: "DISTORTION、FILTER、VOLUME"
  - name: "MXR M75 Super Badass Distortion"
    price: "¥16,500〜"
    amazonUrl: "https://www.amazon.co.jp/s?k=MXR+M75+Super+Badass+Distortion&tag=gearnoteguitar-22"
    soundhouseUrl: "https://www.soundhouse.co.jp/search/index?search_all=MXR+M75&i_type=a"
    ikebeUrl: "https://px.a8.net/svt/ejp?a8mat=4B5Q82+7EQ8KA+5KFA+BW0YB&a8ejpredirect=https%3A%2F%2Fwww.ikebe-gakki.com%2FForm%2FProduct%2FProductList.aspx%3Fshop%3D0%26cat%3D%26bid%3Dec%26dpcnt%3D20%26img%3D1%26sort%3D07%26udns%3D1%26fpfl%3D0%26sfl%3D0%26pno%3D1%26swrd%3DM75%2BMXR"
    ishibashiUrl: "https://px.a8.net/svt/ejp?a8mat=4B5Q82+7FBO62+F14+BW0YB&a8ejpredirect=https%3A%2F%2Fstore.ishibashi.co.jp%2Fsearch%3Fsort%3DNumber1%26q%3Dmxr%2Bm75"
    image: "https://m.media-amazon.com/images/I/81rwLlqGemL._AC_SY300_SX300_QL70_ML2_.jpg"
    specs:
      タイプ: "ディストーション"
      電源: "DC9V"
      コントロール: "OUTPUT、BASS、MID、TREBLE、GAIN"
---

## ディストーションとは？

ディストーションとは、ギターの信号を強く歪ませて、ハードロックやメタルに代表される「ガリガリ・ザリザリ」とした激しいサウンドを生み出すエフェクターです。

**オーバードライブとの違い**はひと言でいうと「歪みの量と質」です。オーバードライブが「アンプを自然に歪ませた音」を再現するのに対し、ディストーションは**意図的にサウンドを大きく変形させた、より激しい歪み**を作ります。ピッキングのニュアンスよりも「常に一定の激しい歪み」が特徴で、ロック・ハードロック・パンクなどのジャンルに最適です。

## ディストーションが必要な理由

「もっとガツンとした音が欲しい」「バンドでロックをやりたい」という人に、ディストーションは必須のエフェクターです。オーバードライブだけでは出せない激しい歪みと音の圧力を、ペダル1つで実現できます。

## 初心者おすすめディストーション3選

### 1. BOSS DS-1（¥9,350〜）

1978年の発売以来、世界で最も売れたディストーションペダルの一つ。カート・コバーン（ニルヴァーナ）やスティーヴ・ヴァイなど、数えきれないほどのギタリストが愛用してきた伝説的な1台です。

「ザリザリ」とした独特の歪みはDS-1ならでは。価格はBOSSのコンパクトエフェクターの中でも最安値クラスで、初心者がロック・パンクのサウンドを手に入れるための最初の選択肢として最適です。

**こんな人に**：最低価格でロックサウンドを手に入れたい初心者。DS-1のザラザラした歪みが好きな人。

### 2. ProCo RAT2（¥14,900〜）

1978年にアメリカで生まれた「RAT」の現行モデル。コートニー・ラブ、ソニック・ユースなどオルタナティブロック系のギタリストに特に愛されてきました。

FILTERノブ（逆回転のトーン調整）による音作りの幅広さが特徴で、クランチ（軽い歪み）からヘビーディストーションまで幅広く対応。プロユースのサウンドを比較的手頃な価格で手に入れられる点が魅力です。

**こんな人に**：オルタナ・パンク・ハードロック系のサウンドが好きな人。音作りの幅広さを重視する人。

### 3. MXR M75 Super Badass Distortion（¥16,500〜）

BASS・MID・TREBLEの3バンドEQを搭載した本格的なディストーション。EQを細かく調整できるため、あらゆるアンプやギターに対応できる汎用性の高さが最大の特徴です。

クリーンミックス機能はないものの、3バンドEQがあることでどんな環境でもベストな音を作れます。「どのペダルを買っても音が決まらない」という人に特におすすめです。

**こんな人に**：細かい音作りがしたい人。さまざまな環境でベストサウンドを出したい人。

## おすすめ製品 比較表

| 製品名 | 価格帯 | 歪みの質感 | 初心者向け度 | こんな人に |
|--------|--------|-----------|------------|-----------|
| BOSS DS-1 | ¥9,350〜 | ザリザリした激しい歪み | ★★★★★ | 最初の1台 |
| ProCo RAT2 | ¥14,900〜 | 幅広い歪みレンジ | ★★★★☆ | オルタナ・パンク好き |
| MXR M75 | ¥16,500〜 | EQで細かく調整可能 | ★★★☆☆ | 音作り重視 |

## ディストーションの基本的な使い方・音作りのコツ

**ノブの役割：**
- **DIST / GAIN**：歪みの量。最初は9時〜12時から試す
- **TONE / FILTER**：音の明るさや重さを調整
- **LEVEL / OUTPUT**：出力音量

**初心者が陥りがちな失敗：**
「DIST（歪み）を最大まで上げればかっこいい音になる」と思いがちですが、実際は歪ませすぎると音がつぶれてバンドの中で聞こえなくなります。まずは控えめに設定して、徐々に好みの歪み量を探しましょう。

## まとめ

最初の1台としては**BOSS DS-1**一択です。9,000円台で本物のロックサウンドが手に入り、耐久性も抜群。「激しすぎる音が苦手」という人はオーバードライブを、「もっと本格的な音が欲しい」という人はRAT2やM75へのステップアップを検討してみてください。

---

## 関連記事

- 👉 [BOSS DS-1 単体レビュー](/articles/boss-ds-1-review)
- 👉 [オーバードライブの選び方・おすすめ3選](/articles/overdrive-guide)
- 👉 [歪み系エフェクター（OD/ディストーション/ファズ）の違いと選び方](/articles/distortion-category-guide)
- 👉 [初心者のエフェクターボード構成ガイド](/articles/beginner-effector-board-guide)
